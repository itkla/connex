...
**Page shell & full width.** Every routed surface that renders inside the app shell `<main>` wraps its content in `PageShell` (`app/components/PageShell.tsx`) — never a hand-rolled `min-h-full … mx-auto max-w-* flex flex-col gap-*` div. `PageShell` takes no width prop, because **there is no page-width cap**: every page spans the full content area at every screen size, so no surface pays for a dead gutter. Do not reintroduce a cap — not through `className`, not through an `mx-auto max-w-*` wrapper sitting immediately inside the shell, and not by hand-rolling a capped column in a board component. `test/unit/loadingStates.test.ts` fails any `<PageShell>` carrying a `max-w-*` and any `mx-auto max-w-*` wrapper rendered as the shell's first child. Both read the source statically, so a cap smuggled through a computed `className` is review-caught, not gate-caught.
...
- **Editors, forms, and readers** span *as pages* — the page header, breadcrumb, background, and dividers reach the full width, the way a document editor's chrome does — while the content inside keeps its own measure: a readable measure on the **text block** (`max-w-3xl` on the note body, `max-w-[70ch]` on generated prose, the report `max-w-6xl` paper) and field grids that flow responsively. A formatting toolbar that belongs to a measured body stays with that body; only page-level chrome spans. A measure on the block is right; the same value on the page is the cap coming back through the back door.
...
Section rhythm (`gap-10` between stacked children) and the page gutter come from the shell — do not re-declare them, and do not override the gap per page: uniform rhythm is the point. The gutter is `px-2` stepping to `2xl:px-6`, on top of `<main>`'s `p-6`, so a full-width page breathes on a large display instead of sitting against the frame; a hand-rolled page shell matches that pair. Full-height editor shells keep their own gutter (`px-4 sm:px-6`) — they are exempt from `PageShell`, and from this pair with it. A `loading.tsx` skeleton renders the same `PageShell` tag as the page it stands in for — a skeleton and its page must never disagree about the wrapper, or the page visibly jumps when data arrives.
...
**The documented D5 adaptations.** Per-surface deviations are decisions, and this is the list of them. Nothing else deviates.

- **Campaigns instant-create asks for two facts, not one.** `NewCampaignDialog.tsx` collects a name *and* a type before landing in `/marketing/campaigns/{id}`, because `POST /api/campaigns` requires a type and no default would be truthful — it is a fact about the customer's marketing, not a Connex mechanism. Everything the 377-line create dialog used to collect (objective, status, budget, window) is the builder's job now. Short as the prompt is, it still accumulates input, so it wires the guard like everything else in the inventory; the guard only interposes once a field is non-empty, so opening it and pressing Escape stays instant.
- **Campaign settings edit is a `QuickEditSheetShell` drawer.** `EditCampaignSheet.tsx` renders the shared quick-edit chrome over the builder, so the campaign stays visible behind the edit, and inherits the guard and the `QuickEditField` error line rather than re-deciding either.
- **The calendar peek expands; it does not shared-element morph.** A click on a fine pointer opens `EventPeekPopover.tsx` anchored to the clicked entry (`PopoverContent` forwards Base UI's `anchor` for exactly this); "Details" then opens `EventDetailSheet.tsx` — a right drawer on desktop, a bottom sheet below `md`. A coarse pointer skips the popover and opens the drawer directly. The two are different overlay primitives with their own CSS enter/exit, so the hand-off is a sequenced expand rather than a fourth morph: continuity comes from an identical header (kind badge, title, when) and the popover's faster exit, not from a shared `layoutId`. Because the peek is anchored to a DOM node the calendar owns, it is cleared whenever the period, view, or type filter changes, and it composes with the keyboard shortcuts: `CalendarShell`'s shortcut guard matches Base UI's `data-open` as well as Radix's `data-state="open"`, so navigation keys no longer fire out from under an open overlay.
- **Products open in place.** `/records/products` runs the full Records-browser standard except peek and a detail route: `PRODUCT.md` §7 keeps products managed through their dialog, so a row click opens `ProductDialog` and `RecordsRenderView` gets no `detailPath`. Its filtering, sorting, and paging are client-side because there is no product page endpoint; saved views, owner scope, custom fields, comment indicators, and bulk tag/owner are all record-type-gated away from products and stay unbuilt rather than faked. Its CSV export is the one place that shows: `GET /api/exports/products` takes only a search, so the export action's copy names the search and says the browser's facets are not applied, rather than promising the view.
- **Record timeline comments are read-only rows.** `Timeline.tsx` carries `comment` entries so a record has one chronology, but `CommentsSection` remains the composer and thread view: a timeline comment row has no edit/delete menu and no `?comment=` deep-link claim of its own — a separate "View in thread" link hands off to the thread, which owns the highlight and the scroll. **The comment text itself is never wrapped in a link.** `NoteContent` renders mentions and record references as anchors, so linking the content would nest anchors, and the SSR parser would auto-close the outer one and mismatch on hydration — every `NoteContent` call site renders inside a `<p>` or `<span>`.
