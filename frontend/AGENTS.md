...
pnpm 11 re-checks every committed lockfile entry against the policy it resolves on `--frozen-lockfile` installs, so while these settings stand, CI and the Docker build fail if a lockfile bypassed them. The workspace file is not pnpm's only configuration source: a `.pnpmfile.cjs` or `.pnpmfile.mjs` beside it (this directory has one) can rewrite every setting from an `updateConfig` hook after the file is read. `../.github/scripts/check-pnpm-supply-chain-policy.py`, run by the Security "Frontend dependency audit" job, therefore fails when a setting is missing or changed; when any top-level key other than `allowBuilds`, `overrides`, these settings, and the two exclusion lists appears (so `pnpmfile`, `configDependencies`, `registry`, or `trustLockfile` needs a reviewed guard change); when a value continues onto an indented line; when a pnpmfile names any hook or export other than `readPackage`, or loads another module; or when an exclusion breaks the rules below. Its `--effective` step then compares what `pnpm config list --json` resolves in each workspace, after pnpmfile hooks run, with the policy, which catches a change to these settings that the file scan cannot see. Never drop or relax these settings, or override them from a pnpmfile, to get an install through.
