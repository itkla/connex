...
- **Segment error boundaries under `app/(app)` recover with Next's `retry`; the root `app/error.tsx` and `app/global-error.tsx` recover with `reset`.** `retry` refreshes the router, which re-publishes its canonical URL, and the one-time-link entry routes sit under the two root boundaries. `test/unit/oneTimeLinkEntryGuard.test.ts` fails CI if a boundary above an entry route reads `retry`; never relax that guard to make a change pass.
...
- **`trustPolicyExclude`** holds only verified legacy-line backports published without trust evidence: one exact `name@x.y.z` per entry, with a comment directly above carrying `Publisher: <npm user>`, `Line: <major>.x`, `Published: <YYYY-MM-DD>` (read them with `pnpm view <name>@<version> --json`: `_npmUser.name` and `time`) and a `Remove …` condition. The guard refuses an entry missing any of them.
