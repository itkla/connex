...
function workflowAuthoring(locale: string): { [key: string]: MessageValue } {
    const parsed: unknown = JSON.parse(
        readFileSync(path.resolve(process.cwd(), "messages", locale, "workspace.json"), "utf8"),
    );
    if (!isMessageTree(parsed) || !isMessageTree(parsed.WorkflowAuthoring)) {
        throw new Error(`messages/${locale}/workspace.json has no WorkflowAuthoring namespace`);
    }
    return parsed.WorkflowAuthoring;
}
...
function missingKeys(locale: string): string[] {
    const authoring = workflowAuthoring(locale);
    const keys = [
    ...
    return [...new Set(keys)].filter((key) => resolve(authoring, key) === null);
}
