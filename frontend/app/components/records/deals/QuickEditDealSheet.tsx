        ...
        <Sheet open={open} onOpenChange={onOpenChange}>
            <SheetContent side="right" className="flex w-full flex-col sm:max-w-lg">
                <SheetHeader className="border-b">
                    <SheetTitle>
                        {selectedIds.size === 1 ? 'Quick edit deal' : `Quick edit ${selectedIds.size} deals`}
                    </SheetTitle>
                    <SheetDescription>
                        Update fields below. Only changed deals will be saved.
                    </SheetDescription>
                </SheetHeader>
                                ...
                                <div key={d.id} className={idx > 0 ? 'border-t pt-6' : ''}>
                                    ...
                                    <div className="grid gap-3">
                                        <div className="grid gap-1.5">
                                            <Label htmlFor={`deal-name-${d.id}`}>Name</Label>
                                            <input
                                                id={`deal-name-${d.id}`}
                                                type="text"
                                                value={draft.name}
                                        ...
                                        <div className="grid grid-cols-[1fr_120px] gap-3">
                                            <div className="grid gap-1.5">
                                                <Label htmlFor={`deal-value-${d.id}`}>Value</Label>
                                                <input
                                                    id={`deal-value-${d.id}`}
                                                    type="number"
                                                    min="0"
                                            ...
                                            <div className="grid gap-1.5">
                                                <Label htmlFor={`deal-currency-${d.id}`}>Currency</Label>
                                                <input
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            <div className="grid gap-1.5">
                                                <Label htmlFor={`deal-pipeline-${d.id}`}>Pipeline</Label>
                                                <Combobox
                                                    items={pipelines}
                                                ...
                                                >
                                                    <ComboboxInput id={`deal-pipeline-${d.id}`} placeholder="Select pipeline" className="ring-1 ring-black/5" />
                                                    <ComboboxContent className="pointer-events-auto">
                                                        <ComboboxList onWheel={handleListWheel}>
                                                            <ComboboxEmpty>No pipelines.</ComboboxEmpty>
                                                            {pipelines.map((p) => (
                                            ...
                                            <div className="grid gap-1.5">
                                                <Label htmlFor={`deal-stage-${d.id}`}>Stage</Label>
                                                <Combobox
                                                    items={stages}
                                                ...
                                                >
                                                    <ComboboxInput
                                                        id={`deal-stage-${d.id}`}
                                                        placeholder={draft.pipeline ? 'Select stage' : 'Pick a pipeline first'}
                                                        disabled={!draft.pipeline}
                                        ...
                                        <div className="grid gap-1.5">
                                            <Label htmlFor={`deal-company-${d.id}`}>Company</Label>
                                            <Combobox
                                                items={companies}
                                            ...
                                            >
                                                <ComboboxInput id={`deal-company-${d.id}`} placeholder="Select company" showClear className="ring-1 ring-black/5" />
                                                <ComboboxContent className="pointer-events-auto">
                                                    <ComboboxList onWheel={handleListWheel}>
                                                        <ComboboxEmpty>No companies.</ComboboxEmpty>
                                                        {companies.map((c) => (
                                        ...
                                        <div className="grid gap-1.5">
                                            <Label htmlFor={`deal-close-${d.id}`}>Expected close date</Label>
                                            <input
                                                id={`deal-close-${d.id}`}
                                                type="date"
                                        ...
                                        <div className="grid gap-1.5">
                                            <Label htmlFor={`deal-close-${d.id}`}>Closed?</Label>
                                            {/* <input
                                                id={`deal-closed-${d.id}`}
                                                type="checkbox"
                                            ...
                                            {draft.closedAt !== null && (
                                                <>
                                                <div className="grid gap-1.5">
                                                    <Label htmlFor={`deal-closed-at-${d.id}`}>Closed at</Label>
                                                    <input
                                                ...
                                                <div className="grid gap-1.5">
                                                    <Label htmlFor={`deal-actual-value-${d.id}`}>Actual value</Label>
                                                    <input
