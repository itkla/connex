 ...
 */
export const WARM_PATH_LIMIT = 50;
...
/** Carries one contact record's shared ask. Defaults to offering none, so a surface used off a
 * record page renders no ask rather than throwing. */
export const ContactIntroAskContext = createContext<ContactIntroAsk>(NO_ASK);

/**
 * The contact record's shared intro ask. Outside a `ContactIntroAskProvider` it reports no bridge,
 * so a surface reused off the record page simply renders no ask.
 */
...
export function bridgeToTarget(
    paths: readonly WarmPath[],
