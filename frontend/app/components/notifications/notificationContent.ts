...
export function notificationContent(notification: Notification, t: Translator, locale: string) {
    ...
    if (notification.type === "activity.mention") {
        return {
            title: t("activityMentionTitle"),
            body: t("activityMentionBody", { actor: notification.actorLabel ?? "" }),
        };
    }
    if (notification.type === "introduction.mention") {
        return {
            title: t("introductionMentionTitle"),
