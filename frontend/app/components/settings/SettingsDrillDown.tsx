 ...
 * the way out is always the same control in the same place.
 *
    ...
    return (
        <div>
                ...
                <div className="mb-2 flex items-center gap-1">
                    <button
                        ...
                        onClick={() =>
                            setPath((current) =>
                                current.group === null
                                    ? { scope: null, group: null }
                                    : { scope: current.scope, group: null },
                            )
                        }
            ...
            <AnimatePresence mode="wait" initial={false}>
                ...
                >
                    {heading ? <h2 className="mb-2 px-3 text-base font-semibold text-foreground">{heading}</h2> : null}
                        ...
                        {level === "scopes"
                            ? scopes.map((candidate) => (
                                  <li key={candidate.scope}>
                                      <button
                                          type="button"
                                          onClick={() => setPath({ scope: candidate.scope, group: null })}
                                          className={ROW_CLASS}
                                      >
                            ...
                            ? scope.groups.map((candidate) =>
                                  candidate.destinations.length > 1 ? (
                                      <li key={candidate.id}>
                                          <button
                                              type="button"
                                              onClick={() => setPath({ scope: scope.scope, group: candidate.id })}
                                              className={ROW_CLASS}
                                          >
