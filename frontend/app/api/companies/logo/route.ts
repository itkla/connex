 ...
 * its public URL. The caller must belong to the workspace that owns the company;
 * persisting the returned URL is still done via the backend by the caller.
 */
export async function PUT(request: NextRequest) {
    ...
    if (!(await workspaceCanAccessEntity(cookie, "company", Number(companyId)))) {
