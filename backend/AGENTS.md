...
- **Provider egress is validated and re-vetted.** Bedrock hosts derive from the closed `BedrockRegion` enum and use `AiEgressGuard.requireAllowlistedHost` immediately before every send. Azure OpenAI reconstructs HTTPS URLs from suffix-pinned `*.openai.azure.com` hosts and uses `AiEgressGuard.requireFetchableHost(host, false)` immediately before every send. OpenAI-compatible adapters may use the organization-configured HTTP(S) base URL, but must call `AiEgressGuard.requireFetchableHost` immediately before every send and honor `allowInternalEndpoint`; redirects remain disabled. New providers get their own adapter behind `AiProvider` and register through `AiProviderRouter`.
