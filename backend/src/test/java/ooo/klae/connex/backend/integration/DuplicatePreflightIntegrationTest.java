...
class DuplicatePreflightIntegrationTest {
    ...
    @Test
    void interactiveCreationRequiresServerSideDuplicateAcknowledgement() throws Exception {
        RequestContextHolder.resetRequestAttributes();
        Workspace workspace = newWorkspace(newOrganization(), "reviewed-create");
        User owner = newMember(workspace, "owner");
        Person person = newPerson(workspace, "Existing Reviewed Person");
        insertPersonIdentity(person, "reviewed-person@example.com");
        newCompany(workspace, "Existing Reviewed Company");
        MockHttpSession session = login(owner.getUsername());
        String personReviewToken = reviewToken(
            workspace,
            session,
            "/api/duplicate-preflight/persons",
            """
            {
              "name": "New Reviewed Person",
              "emails": ["reviewed-person@example.com"],
              "phones": []
            }
            """);

        String unacknowledgedPerson = """
            {
              "name": "New Reviewed Person",
              "email": "reviewed-person@example.com",
              "phone": "",
              "title": "",
        ...
        mockMvc.perform(post("/api/persons")
                ...
                .session(session)
                .contentType(MediaType.APPLICATION_JSON)
                .content(unacknowledgedPerson.replace(
                    "\"duplicateReviewToken\": null",
                    "\"duplicateReviewToken\": \"" + personReviewToken + "\"")))
            .andExpect(status().isOk())
            .andExpect(content().string(Matchers.not(
                Matchers.containsString("duplicateReviewToken"))));

        String companyReviewToken = reviewToken(
            workspace,
            session,
            "/api/duplicate-preflight/companies",
            """
            {
              "name": "Existing Reviewed Company",
              "websites": [],
              "phones": []
            }
            """);
        String unacknowledgedCompany = """
            {
              "name": "Existing Reviewed Company",
              "website": null,
              "industry": null,
        ...
        mockMvc.perform(post("/api/companies")
                ...
                .session(session)
                .contentType(MediaType.APPLICATION_JSON)
                .content(unacknowledgedCompany.replace(
                    "\"duplicateReviewToken\": null",
                    "\"duplicateReviewToken\": \"" + companyReviewToken + "\"")))
            .andExpect(status().isOk())
            .andExpect(content().string(Matchers.not(
                Matchers.containsString("duplicateReviewToken"))));
    }
