 ...
 * have been mutated. Recipients are deduplicated and ordered so a transaction acquires each state
 * row at most once and concurrent multi-recipient flows use a stable lock order.
 */
@Component
...
public class NotificationStateVersionService {
    ...
    public void markChanged(int recipientId) {
        ...
        recipientVersions().add(recipientId);
    }
    ...
    public long bumpNow(int recipientId) {
        ...
        notificationMapper.bumpStateVersions(List.of(recipientId));
        return notificationMapper.getStateVersion(recipientId);
    }
