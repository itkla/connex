...
| Automated backups and point-in-time recovery | Inherited from the encrypted source database, with restore drills proving restored targets stay encrypted. | Connex and AWS. | Backup/PITR configuration plus restore evidence showing encrypted target database and expected KMS key. |
| Manual snapshots and snapshot copies | Encrypted. Same-region snapshots use the expected source key boundary; cross-region copies, if ever approved, specify a destination-region KMS key. | Connex and AWS. | Snapshot inventory with `Encrypted=true`, expected `KmsKeyId`, non-public sharing posture, copy policy, and retention owner. |
...
| S3 buckets for attachments, imports, exports, backup staging, and support artifacts | Bucket default encryption using SSE-KMS or DSSE-KMS with Connex-managed KMS keys for production buckets; bucket policy denies unencrypted writes and non-TLS access where service integrations allow it. | Connex and AWS. | Bucket encryption configuration, bucket policy, replication encryption config where used, object sample checks, and retention lifecycle policy. |
...
| CloudWatch Logs groups and stored query results that may contain customer data | CloudWatch Logs default at-rest encryption at minimum; production application/security log groups must associate the Connex production KMS key unless the log group is proven to contain no customer data. | Connex and AWS. | Log group inventory with `kmsKeyId`, retention days, and owner; exception record for any group without KMS association. |
...
aws rds describe-db-instances \
  ...
  --query 'DBInstances[].{DBInstanceIdentifier:DBInstanceIdentifier,StorageEncrypted:StorageEncrypted,KmsKeyId:KmsKeyId,DBClusterIdentifier:DBClusterIdentifier,ReadReplicaSourceDBInstanceIdentifier:ReadReplicaSourceDBInstanceIdentifier}'

aws rds describe-db-clusters \
  --region ap-northeast-1 \
  --query 'DBClusters[].{DBClusterIdentifier:DBClusterIdentifier,StorageEncrypted:StorageEncrypted,KmsKeyId:KmsKeyId,ReplicationSourceIdentifier:ReplicationSourceIdentifier}'
...
or has pre-existing objects outside the required encryption mode.
...
aws logs describe-log-groups \
  --region ap-northeast-1 \
  --query 'logGroups[].{logGroupName:logGroupName,kmsKeyId:kmsKeyId,retentionInDays:retentionInDays}'
```
...
or no retention owner. Treat logs as customer-data-adjacent even when the
application is designed not to log personal data.
...
aws backup list-backup-vaults \
  --region ap-northeast-1 \
  --query 'BackupVaultList[].{BackupVaultName:BackupVaultName,EncryptionKeyArn:EncryptionKeyArn,Locked:Locked}'

aws backup list-recovery-points-by-backup-vault \
  --region ap-northeast-1 \
  --backup-vault-name "$VAULT" \
  --query 'RecoveryPoints[].{RecoveryPointArn:RecoveryPointArn,ResourceType:ResourceType,ResourceArn:ResourceArn,EncryptionKeyArn:EncryptionKeyArn,CreationDate:CreationDate,Status:Status}'
```
...
cross-region copy lacks destination-key evidence.
...
| Rule | Purpose |
| --- | --- |
| `rds-storage-encrypted` | DB instances must have storage encryption enabled. |
| `rds-snapshot-encrypted` | RDS snapshots must be encrypted. |
| `s3-bucket-server-side-encryption-enabled` | S3 buckets must have default encryption or a policy that denies unencrypted writes. |
...
| Backup/PITR encryption evidence | |
| Manual snapshot evidence | |
| Replica/restore target evidence | |
| S3 customer-data buckets and key ARNs | |
| Log groups and key ARNs | |
...
2. AWS Config or equivalent continuous checks are enabled for database,
   snapshot, S3, and backup encryption.
3. Every database, backup, snapshot, replica, restore target, bucket, log group,
   and customer-data artifact path has an expected encryption mode and owner.
