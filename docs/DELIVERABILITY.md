...
| Campaign unsubscribe link in the email body is unclickable | `connex.delivery.public-base-url` is unset, so the URL is emitted as the relative link `/unsubscribe#token={token}` | Set it to the instance's absolute public base URL before sending any campaign |
