...
- Do not describe a Connex-operated dedicated silo as E2EE, zero-knowledge,
  customer-only-key encrypted, or a deployment where Connex cannot access data;
  a silo is a Connex-operated environment. Do not say a silo gives the
  "Connex cannot see our data" guarantee — qualify it to customer-operated/on-prem.
...
| Dedicated per-organization database tier | Future architecture only; it is not currently available for sale or contractual commitment. If implemented, dedicated database isolation would use the hosted storage-encryption posture in `SAAS_STORAGE_ENCRYPTION_RUNBOOK.md`. A dedicated logical database is not a customer-managed-key tier. | Not currently available. A future implementation would remain Connex-controlled by default; customer-managed key custody would require separate silo infrastructure and every feasibility gate in `DEDICATED_SAAS_CMK_FEASIBILITY.md`. | Not currently available. Any future dedicated-CMK implementation must prove provider-specific disablement, recovery, and restore behavior before the capability is offered. | If implemented, the Connex backend would still process plaintext while the dedicated environment is running and keys are available. | Future dedicated snapshots/backups must stay inside the same tenant/key boundary, and restore, blue-green, and repointing flows must verify the target posture before traffic moves. | "Dedicated database and dedicated CMK tiers are not currently available. Their feasibility and launch gates are documented, but they must not be sold or described as shipped." |
| Connex-operated dedicated silo (dedicated single-tenant instance + database + isolated network) | Feasible future topology, not a currently available product. A silo could provide dedicated infrastructure and network isolation and could use an organization's selected KMS key only after every gate in `DEDICATED_SAAS_CMK_FEASIBILITY.md` is implemented and verified. | Not currently available. A future silo would be operated by Connex; customer-managed key custody would be optional only where separately contracted and evidenced. | Not currently available. A future contracted CMK mode must prove that key disablement makes the silo unavailable and that controlled recovery works. | If implemented, Connex would operate the silo, so its backend would still process plaintext and privileged Connex operational access could reach it under the signed support/access model. | Future silo snapshots, backups, replicas, logs, and restore targets must stay inside the organization's evidenced key boundary. | "A Connex-operated dedicated silo is a future feasible topology, not a currently available product. Even if implemented with customer-managed storage keys, it would not be E2EE or zero-knowledge." |
...
A future Connex-operated dedicated silo would go further than the
dedicated-database tier: dedicated single-tenant infrastructure plus network
isolation would make per-organization customer-managed storage keys feasible
where the shared-server dedicated-database tier cannot. Because Connex would
still operate the silo, it would provide tenant-from-tenant isolation, dedicated
blast-radius, and optional customer key custody. Even if implemented, a silo
does not make Connex blind to running data. The "Connex cannot see our data"
guarantee remains customer-operated/on-prem only, using the same deployable
artifact operated by the customer instead of Connex.
...
| Does Connex support customer-managed keys? | Hosted pooled SaaS does not provide customer-managed database keys, and dedicated Connex-operated database/CMK tiers are not currently available. A future dedicated silo could support customer-managed storage keys only after every gate in `DEDICATED_SAAS_CMK_FEASIBILITY.md` is implemented and verified; it would still not provide application blindness. Customer-operated/on-prem deployments can use customer-controlled keyring/KMS/HSM/KMIP/Vault integrations outside the Connex application package. |
...
| What happens if a customer revokes a key? | For hosted Connex-operated searchable CRM data, customer key revocation is not currently available. Any future dedicated CMK tier would need to implement and prove the behavior in `DEDICATED_SAAS_CMK_FEASIBILITY.md` before being offered. For customer-operated MySQL/InnoDB keys, revocation or withholding can lock the database/application out when the database needs the key. For full-volume encryption, revocation usually affects attach, restart, restore, or snapshot decryptability and may not immediately stop an already-running mounted database. For Connex secret-store credentials, revocation makes only those wrapped credentials unavailable. |
...
- Deployment architecture / Connex-operated silo + on-prem planning ([#100]).
