...
| Connex-operated dedicated silo (dedicated single-tenant instance + database + isolated network) | Dedicated single-tenant infrastructure: a dedicated database instance/cluster and isolated network, provisioned under the organization's selected KMS key where contracted. Because storage is dedicated infrastructure — not a `cnx_<random>` logical database on a shared server — customer-managed keys (BYOK/CMK) are feasible per `DEDICATED_SAAS_CMK_FEASIBILITY.md`. | Connex operates the silo. Keys are Connex-managed by default; customer-managed key custody is available on contracted silo infrastructure whose storage, backups, snapshots, replicas, and restore targets are created and verified under the organization's KMS key. | Available when a customer-managed key is contracted; disabling or revoking that key makes the silo unavailable per the provider-specific recovery process. Not available under Connex-managed keys. | Connex operates the silo, so the Connex backend still processes plaintext and privileged Connex operational access can reach it under the support/access model in the signed agreement. The isolation bought is tenant-from-tenant plus dedicated blast-radius and network isolation — not Connex-from-data. "Connex cannot see our data" remains customer-operated/on-prem only. | Silo snapshots/backups stay inside the organization's dedicated key boundary; restore, blue-green, and repointing must verify the target posture before traffic moves. Logical exports are plaintext at generation unless separately encrypted. | "A Connex-operated dedicated silo gives dedicated single-tenant infrastructure, network isolation, and optional customer-managed keys, but Connex operates it and the running app still processes plaintext. It is not E2EE, zero-knowledge, or a deployment where Connex cannot access data." |
...
A Connex-operated dedicated silo goes further than the dedicated-database tier:
dedicated single-tenant infrastructure plus network isolation, which makes
per-organization customer-managed storage keys feasible where the shared-server
dedicated-database tier cannot. Because Connex still operates the silo, it buys
tenant-from-tenant isolation, dedicated blast-radius, and optional customer key
custody — but it does not make Connex blind to running data. The "Connex cannot
see our data" guarantee is reserved for customer-operated/on-prem, which is the
same deployable artifact operated by the customer instead of Connex.
...
| Does Connex support customer-managed keys? | Hosted pooled SaaS does not provide customer-managed database keys. Dedicated per-organization CMK is available only on contracted Connex-operated silo infrastructure (a dedicated single-tenant instance/cluster, not a `cnx_<random>` dedicated logical database on a shared server) that satisfies `DEDICATED_SAAS_CMK_FEASIBILITY.md`. A silo is Connex-operated, so customer key custody there is storage-layer custody, not application blindness. Customer-operated/on-prem deployments can use customer-controlled keyring/KMS/HSM/KMIP/Vault integrations outside the Connex application package. |
