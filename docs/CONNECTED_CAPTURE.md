...
Current-workspace purge requires recent authentication and removes the user's provider-owned activities, source envelopes, participants, decisions, policy, and cursor state. Disconnect first marks the control connection as `disconnecting`, which prevents new claims, then advances an owner-leased durable workspace cursor through bounded purge pages before best-effort provider revocation and generation-checked credential deletion. A failed catalog purge remains `purge_failed` and resumes after the last committed workspace; the credential is retained until cleanup can finish.

Membership removal purges that user's captured content in the affected workspace. Account deletion iterates the active tenant catalogs directly and purges the user's captured content across every workspace for both providers even when a connection row is already absent. Tenant export and teardown include every capture table through the tenant lifecycle registry; teardown residual verification must be clean before control roots are removed. Metadata-only audit records may state that a policy, review action, purge, or disconnect occurred, but retained audit data contains no token, source content, participant identity, subject, or body.
