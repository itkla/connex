...
Cloud infrastructure (AWS, `ap-northeast-1`) and, only when a customer enables it, the customer-designated AI provider under BYOP ([#94]), where the customer holds the provider contract and is the data exporter. Cloudflare is selected for the public SaaS edge but must not process production traffic until vendor/privacy review, the DPA subprocessor annex, customer notice, and the applicable cross-border safeguards are complete as required by [EDGE_DEFENCE.md](EDGE_DEFENCE.md). The active list is maintained in the signed DPA and available to customers on request.
