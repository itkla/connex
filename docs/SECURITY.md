...
- **Local business-card OCR:** uploaded cards are globally admission-limited, size/signature/full-decode/dimension/frame bounded, and passed to a private bearer-authenticated CPU-only PaddleOCR sidecar. Malformed import idempotency keys, principals without the import entitlement, and principals over the fixed import-admission window are rejected before import multipart parsing. The sidecar has no ingress route, outbound network access, remote-image fetch, runtime model download, content logging, or unbounded queue; its pinned BOS model artifacts are SHA-256 verified at build time, and its read-only container is capped at two CPUs and 2 GiB. Unsupported CPUs fail startup without processing card data. A hard inference watchdog marks the service unavailable and terminates a wedged native worker so the container restart policy replaces it; bounded 422 image rejections, including excess recognized lines, do not restart the worker. OCR returns only bounded recognized lines for review, and raw OCR output is not persisted. Confirmed imports use owner-bound workspace-scoped UUID idempotency claims plus SHA-256 request fingerprints; fingerprints and request bodies are never logged, and key reuse with different card bytes or reviewed fields fails closed.
