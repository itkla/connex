...
- **Local business-card OCR:** uploaded cards are size/signature/full-decode/dimension/frame bounded before a private bearer-authenticated CPU-only PaddleOCR sidecar sees them. The sidecar has no ingress route, remote-image fetch, runtime model download, content logging, or unbounded queue; its read-only container is capped at two CPUs and 2 GiB. OCR returns only bounded recognized lines for review, and raw OCR output is not persisted. Confirmed imports use workspace-scoped UUID idempotency claims plus SHA-256 request fingerprints; fingerprints and request bodies are never logged, and key reuse with different card bytes or reviewed fields fails closed.
