...
| `csp-enforcement.spec.ts` | the shipped Content Security Policy is actually *enforced*: authenticated dashboard/analytics/records/library/Ask Connex journeys, an upload and its same-origin preview, the dark theme, and the logged-out login/register/document-acceptance pages all raise zero `securitypolicyviolation` events, while an injected un-nonced inline script is blocked and delivered to `/api/csp-reports` as a 204 |

`playwright.config.ts` launches Chromium with `--short-reporting-delay` so the Reporting API delivers a violation report in about 100 ms instead of its default ~1 minute schedule; without it the CSP probe test would spend its whole budget waiting for the delivery.
