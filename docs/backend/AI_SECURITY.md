...
- **A routed skill seeds its declared toolsets, inside the same branch that writes the attribution.** `SkillSpec.toolsets` names the non-core sets a declaration's synthesis step starts holding, so a skill does not spend a governance step rediscovering a read it already declared; `AiSkillCatalogTest` refuses a declaration whose `allowedTools` name a catalog tool that `core` plus its own toolsets never reaches. The seed is applied only inside `if (execution.executed())`, beside `persistenceService.applySkill`, because a reader rebuilding the loaded set from `ai_chat_turn.skill_key` plus the turn's `find_tools` rows must see the set the turn really held: a routed-but-unexecuted plan leaves a null `skill_key` **and** a core-only turn. Keep the two writes together, and resolve the seed **before** the attribution row: the key lookup fails closed, so resolving it second would persist an attribution for a turn that then died holding nothing. **That reconstruction is valid only while `ai_chat_turn.skill_version` equals the version the catalog declares for that key today.** Declarations are mutable across releases — a key is stable and additive, its `toolsets` are not — and `AiSkillCatalog` exposes only `find(String key)`, so a turn recorded under an older version cannot be resolved back to the declaration that actually ran. A reader that sees a version mismatch must fail closed rather than reconstruct from the current declaration; a version-pinned catalog lookup is the alternative, and P0.4 must add one before it consumes this contract for anything but a same-release turn. Seeding grants no authority — `requireSkillAuthority` still owns writes — and a declaration may not seed a write family its `authority` or `allowedTools` cannot call: seeding is all-or-nothing over an object family while write authority is per tool, so `SkillSpec`'s compact constructor refuses that declaration rather than let the server offer a write tool whose use settles the turn as a non-closable `tool_outside_skill_authority`. Every shipped skill declares `Set.of()` today, so this is the forward contract Phase 1's write skills consume rather than a change to what current turns send.
...
### Operator-declared parallel tool calls

One model step carries one tool call unless an operator has declared, for one exact endpoint, that it carries more. The declaration exists; **the agent loop does not execute a batch yet**, and this section states both what ships today and the contract the loop will be held to when it does.

- **The declaration is per endpoint and default off.** `connex.ai.model-overrides[i].parallel-read-calls` binds to `AiProperties.ModelOverride.parallelReadCalls`, is bounded to 1..`AiProviderCapabilities.MAX_PARALLEL_TOOL_CALLS` by bean validation, and is honoured **only together with `endpoint`** — the same character-for-character scoping `streaming` and `thoughts` use, because the same model id behind two gateways is two different answers. `AiModelCatalog.parallelReadCalls` resolves it through the one `endpointDeclared` fold and answers 1 for every undeclared target, so merging the declaration changes nothing anywhere.
- **The bound travels on the request, not in two places.** `AiProvider.parallelToolCallLimit` defaults to 1 for every adapter — Azure, Bedrock and Vertex declare no native tools at all, so the default is defence in depth rather than an enablement — and `OpenAiCompatibleAdapter` answers it from the catalog. The turn snapshots it once into `AiChatMemory.parallelToolCalls` before its first step, so an operator changing the declaration mid-turn cannot move the bound under a turn that already sent requests under the old one, and it rides on `AiNativeToolRequest.maxParallelCalls` so the adapter that serializes the request field and the parser that bounds the response read one value.
- **The wire difference is exactly one field.** `parallel_tool_calls` has always been sent as the literal `false`; it is sent true only when the request's bound is above one. `AiInvocationService.parseNativeToolCall` refuses a response above the bound under the existing `native_multiple_calls` repair rule, so an over-delivering provider is still corrected rather than crashing its turn, and with every endpoint undeclared the bound is 1 and that refusal is byte-for-byte today's.
- **A response is validated whole or not at all.** Every call's arguments are parsed, passed through the raw step guard and demasked individually; a single rejected call makes the whole response malformed, because the guard is the server's statement about one model decision and keeping the calls that happened to be valid would execute part of a decision it refused. Demask warnings are summed across the calls and a non-zero total still fails the turn, so one call inventing a placeholder cannot let its siblings through. Identifiers must be distinct **within** the response as well as against replayed exchanges, under `native_duplicate_call_id`, because the replayed `tool` messages correlate on exactly that identifier. One response is one model call, so it emits one `ai.llm.call` audit row whatever its cardinality.
- **Until the loop can execute a batch, it refuses one.** `AiChatAgentLoopService.nativeStepAttempt` turns a response carrying several calls into the same repairable `native_multiple_calls` refusal, and nothing is proposed, executed or recorded. Executing the first call of a decision the model made as four would run it under an ownership, deadline, authorization and budget admission that were polled once for the whole step. The loop may only start executing batches together with per-call `ownership.isStopped()`, per-call deadline re-polling, per-call `requireCurrentToolExecution`, per-call tool-result budget admission, whole-step admission ordered after `requireSkillAuthority`, and whole-step replay atomicity.
- **The durable key shape is already settled and needs no migration.** A call that was the only one its step made keeps the exact `turn-{turnId}-step-{stepNumber}` key; a call that shared its step renders a `-call-{k}` suffix with k from 1. The longest possible key sits well inside the existing `VARCHAR(64)` column and its `(workspace_id, idempotency_key)` uniqueness, and the `turn-{id}-step-` prefix scan is unchanged. Ordinals are positions of the **emitted** calls: dense in the replay, and potentially **sparse in the durable rows**, because a call answered from the tool-result cache writes no row at all. No reader may infer a call count from row keys. Every site that builds or parses a key bounds the ordinal by `MAX_PARALLEL_TOOL_CALLS`, so a key naming a position no step could have produced is treated as malformed rather than trusted for its step number.
- **The progress projection reads rows, not steps.** `AiChatProgressService` passes `AiChatAgentLoopService.MAX_TOOL_CALL_ROWS_PER_TURN` — the step ceiling times the per-step call ceiling — as its row limit. The old limit was exact only while one step wrote one row, and exceeding it is silent: every suffixed key still parses and every milestone still looks healthy while the rows past the limit are simply absent.

**Probe checklist — record this before declaring any endpoint.** There is no provider credential in any development or CI environment, so nothing in this repository can speak for a real endpoint. The request field itself is known to be accepted, because it already travels on every request; what has to be established is behavioural.

1. Send one native-tools request to the exact endpoint with the parallel field set true and a prompt that plainly needs two independent reads. Record the endpoint string verbatim, the model id, and the date.
2. Confirm the assistant message carries **more than one** entry in `tool_calls`, that each entry has its own non-empty `id`, and that those ids are distinct within that one message.
3. Confirm each entry carries its own opaque replay state where the endpoint uses one — for Gemini's compatibility layer, a per-call thought signature — rather than one signature for the whole message.
4. Replay that assistant message back: all of its `tool_calls` in one message, followed by one `tool` message per call in the same order. Confirm the endpoint accepts it and continues the conversation. An endpoint that emits several calls but rejects that replay cannot be declared.
5. Record steps 1-4, including the cardinalities actually observed, beside the deployment's configuration. Declare `parallel-read-calls` only for that endpoint and model, never deployment-wide, and never above what the probe observed.
