...
- **Every declared tool names one `AiAssistantToolCatalog.Toolset`.** `CORE` — record, activity, task, and plan reads, including `list_scope_activities` and `list_tasks` — is the toolset a turn always holds; the rest group by object family (`analytics`, `schedule`, `write_activity`, `write_content`, `write_pipeline`) so one family can grow without dragging every other write tool along. The keys are stable wire vocabulary: add, never rename. A declaration without a toolset is refused in the `ToolSpec` constructor, because every catalog view filters on it.
- **The taxonomy is declared, not yet wired.** `MAX_ACTIVE_TOOLSETS_PER_TURN` is the single budget for the non-core toolsets one turn may hold — seeded and acquired sets spend from it together — but nothing loads or seeds a toolset yet: `SkillSpec.toolsets` is a declared contract that no call site reads, and `AiChatMemoryService` still measures the one per-turn envelope from `AiAssistantToolCatalog.ALL`, so every turn sends the same prompt, step schema and native definitions as before. `reservationToolsets()` — core plus the `MAX_ACTIVE_TOOLSETS_PER_TURN + RESERVATION_HEADROOM_TOOLSETS` weightiest loadable sets — exists and is measured, but is not what the budget is derived from until on-demand loading lands. Until it is, **adding a tool to any toolset, core or not, grows the envelope every turn actually sends**; check `AiAssistantPromptEnvelopeTest`, which measures the true envelope for every reachable combination on both protocols rather than trusting the catalog weight proxy, and whose per-toolset ceiling is derived from the floor margin the reservation has to fund.
